...
#define RS__PIN   PORTBbits.RB5
#define E__PIN    PORTBbits.RB4
...
extern void EscreveInteiroLCD(int valor);
 ...
 extern void EscreveFraseRomLCD(const char *buffer);
...
{int i;
	...
	RS__PIN = 0;		// comando=0, dados=1
	E__PIN =  0; 	// desabilita=0, habilita=1
...
void EscreveFraseRomLCD(const char *buffer)
{
	while(*buffer)                  // escreve dados no LCD ate achar caracter nulo
	{
	        EscreveCaractereLCD(*buffer); // escreve caracter no LCD
